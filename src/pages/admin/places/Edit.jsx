...
function PlaceEdit() {
  ...
  const getPlaceById = async () => {
    ...
    await Api.get(`/api/admin/places/${id}`, {
      ...
      // Memeriksa apakah latitude dan longitude memiliki nilai sebelum diset ke state
      if (response.data.data.latitude && response.data.data.longitude) {
        setLatitude(response.data.data.latitude);
        setLongitude(response.data.data.longitude);
      } else {
        // Lakukan sesuatu jika latitude atau longitude tidak memiliki nilai
        // Contohnya, mungkin memberikan nilai default atau pesan kesalahan
        setLatitude(-6.451786);
        setLongitude(106.742587);
        // atau
        // console.log("Data latitude atau longitude kosong");
      }
    });
  };
